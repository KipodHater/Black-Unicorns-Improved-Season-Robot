...
public class SimpleCommands {
  ...
  //   private static boolean moveToDefault(Arm arm, Pivot pivot) {
  ...
  private static boolean moveToScore(Arm arm, Pivot pivot) {
    arm.setArmGoal(ArmStates.MIDDLE_OUTTAKE); // can possibly add here multiple angles
    pivot.setPivotGoal(PivotStates.MIDDLE_OUTTAKE);

    return pivot.atGoal() && arm.atGoal();
  }

  public static Command moveToScoreCommand(Arm arm, Pivot pivot) {
    return new FunctionalCommand(
        () -> {},
        () -> {},
        interrupted -> {},
        () -> moveToScore(arm, pivot), // check if finished
        arm,
        pivot);
  }

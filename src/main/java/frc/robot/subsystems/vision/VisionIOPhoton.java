...
public class VisionIOPhoton implements VisionIO {
    ...
    public void updateInputs(VisionIOInputs inputs) {
        ...
        for(var result : results) {
            if (result.hasTargets() && RobotController.getFPGATime() * 1e-6 - result.getTimestampSeconds() < 0.02) {
            ...
            if(result.multitagResult.isPresent()){
                ...
                Transform3d fieldToRobot = robotToCamera.inverse().plus(fieldToCamera); //vector addition
                ...
                for (var target : result.targets) {
                    ...
                    minTagDistance = Math.min(minTagDistance, target.bestCameraToTarget.getTranslation().getNorm());
                  }
                ...
                if (tagPose.isPresent()) {
                  ...
                  Transform3d fieldToRobot = fieldToCamera.plus(robotToCamera.inverse());//vector addition
                  Pose3d robotPose = new Pose3d(fieldToRobot.getTranslation(), fieldToRobot.getRotation());
        ...
        if(!poseObservations.isEmpty()) {
            ...
            if(inputs.poseObservations[poseObservations.size()-1].minimumTagDistance() < lowResTriggerDistance) {
                setHighResPipeline(false); // Switch to low res if the last observation is close enough
            } else if(inputs.poseObservations[poseObservations.size()-1].minimumTagDistance() > highResTriggerDistance){
